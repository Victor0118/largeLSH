...
The psudocode of our distributed KNN algorithm implementation can be viewed in Algorithm~\ref{knn}. In the line 3, instead of iteratively going through each query, the batch-query is run on each cluster through the allocation of Apache Spark, which largely takes advantage of node number. 
In the line 7, $findTopkByPartition()$ is a function transform the pair-wise similarity table to query-wise ranked table by similarity. Apache Spark implements the per-query ranking job in a MapReduce style. 
