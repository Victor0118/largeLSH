...
\begin{align*}
g(v) = h_{a,b}(v) = \floor{\frac{(a\cdot v + b)}{w} }
\end{align*}
...
\begin{algorithm}
	...
	resultsThres $\leftarrow$ threshold(, $d$) \\
	resultsSelected $\leftarrow$ SELECT trainID testID distance FROM results\_thres \\
	resultsPartitioned $\leftarrow$  findTopkByPartition($k$) \\
	$L$ = resultsPartitioned.map(IntersectionVector())
